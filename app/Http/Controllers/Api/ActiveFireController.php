...
class ActiveFireController extends Controller
{
    ...
    public function setViirnsSnppNrtTableData(){
        ...
        $responseViirsSnppNrt = Http::get('https://firms.modaps.eosdis.nasa.gov/api/area/csv/5cd8bf082b0eb8dbc40701e0633c65b5/VIIRS_NOAA21_NRT/world/1/'.$currentDate ); //2024-06-01');
        $csvData = $responseViirsSnppNrt->body();
        if(!empty($csvData)){
            $rows = explode("\n", trim($csvData));
            $header = str_getcsv(array_shift($rows));
        ...
        $responseViirsSnppNrt = Http::get('https://firms.modaps.eosdis.nasa.gov/api/area/csv/5cd8bf082b0eb8dbc40701e0633c65b5/VIIRS_NOAA20_NRT/world/1/'.$currentDate ); //2024-06-01');
        $csvData = $responseViirsSnppNrt->body();
        if(!empty($csvData)){
            $rows = explode("\n", trim($csvData));
            $header = str_getcsv(array_shift($rows));
