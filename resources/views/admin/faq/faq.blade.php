            ...
            <div class="card-header border-0 pt-6">
                ...
                <div class="card-toolbar">
                    ...
                    <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
                        <!--begin::Add customer-->
                        <button type="button" class="btn btn-primary" data-role='addFaq' data-bs-toggle="tooltip" data-bs-dismiss="click" title="Add faq" data-bs-custom-class="tooltip-dark">Add</button>
                        <!--end::Add customer-->
                    </div>
                    <!--end::Toolbar-->
                    <!--begin::Group actions-->
                    ...
                    <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4" id="kt_datatable">
                        ...
                        <tbody class="fw-bold text-gray-600">
                            @if(count($list)>0)
                                ...
                                <td><?php echo stringReadMoreInline($item->question,40)?></td>
                                <td width="50%"><?php echo stringReadMoreInline($item->answer,60)?></td>
                                <td class="text-end">
                                    <div class="d-flex gap-3">
                                        <a href="#" data-id="{{ $item->id }}"
                                            data-title="{{ $item->title }}" data-content="{{ $item->content }}"
                                            data-role="editFaq" data-question="{{$item->question}}" data-answer="{{$item->answer}}" data-bs-toggle="tooltip" data-bs-dismiss="click" title="Edit faq" data-bs-custom-class="tooltip-dark">
                                        ...
                                        <a href="#" id="deleteCatId" data-role="deleteEmailContaint"
                                            data-id="{{ $item->id }}" data-bs-toggle="tooltip" data-bs-dismiss="click" title="Delete faq" data-bs-custom-class="tooltip-dark">
                    ...
                    <div class="scroll-y me-n7 pe-7" id="kt_modal_add_faq_scroll"
                        ...
                        <div class="fv-row mb-7">
                            ...
                            <div class="fv-row mb-7 mt-1">
                                ...
                                <!--begin::Input-->
                                {{-- <input type="text" class="form-control" placeholder="Please write question here"
                                    name="question" id="question" value=""  /> --}}
                                    <textarea class="form-control" name="question" id="question" maxlength="1000" placeholder="Please write question here" rows="3"></textarea>
                                     <span id="charCountQue" class="badge badge-primary">0/1000</span>
                                    {{-- <textarea class="form-control" name="answer" id="answer" maxlength="1000" placeholder="Please write answer here" rows="3"></textarea>
                                     <span id="charCount" class="badge badge-primary">0/1000</span> --}}
                                <!--end::Input-->
                            </div>
                            ...
                            <div class="fv-row mb-7 mt-1">
                                ...
                                <!--begin::Input-->
                                {{-- <input type="text" class="form-control" placeholder="Please write answer here"
                                    name="answer" id="answer" value="" /> --}}
                                    {{-- <textarea class="form-control" name="answer" maxlength="10" placeholder="Please write answer here" id="kt_docs_maxlength_textarea" rows="6"
                                     {{-- onkeyup="lengthCount(this)"
                                     ></textarea> --}}
                                     {{-- <textarea class="form-control" name="question" id="question" maxlength="1000" placeholder="Please write question here" rows="3"></textarea> --}}
                                     {{-- <span id="charCountQue" class="badge badge-primary">0/1000</span> --}}
                                     <textarea class="form-control" name="answer" id="answer" maxlength="1000" placeholder="Please write answer here" rows="3"></textarea>
                                     <span id="charCount" class="badge badge-primary">0/1000</span>
                                    {{-- <span class="position-absolute top-100 start-100 translate-middle  badge badge-circle badge-success" id="countChar"></span> --}}
                                <!--end::Input-->
...
<script>
    ...
    //     if(length == maxlength){
    //         $('#answerId').removeClass('badge badge-success');
    //         $('#answerId').addClass('badge badge-success');
    //     }
    // }
    $(document).ready(function() {
        ...
        // });  $(document).ready(function() {
            var maxChars = $('#answer').attr('maxlength');
        ...
        $('#answer').on('input', function() {
            ...
            // Apply styles based on the character count
            if (currentChars >= maxChars) {
                $('#charCount').removeClass('badge-primary').addClass('badge-danger');
            } else {
                $('#charCount').removeClass('badge-danger').addClass('badge-primary');
            }
        });
            var maxCharsQue = $('#question').attr('maxlength');
        // Update character count
        $('#question').on('input', function() {
            var currentCharsQue = $(this).val().length;

            // Update character count text
            $('#charCountQue').text(currentCharsQue + ' / ' + maxCharsQue);

            // Apply styles based on the character count
            if (currentCharsQue >= maxCharsQue) {
                $('#charCountQue').removeClass('badge-primary').addClass('badge-danger');
            } else {
                $('#charCountQue').removeClass('badge-danger').addClass('badge-primary');
            }
        });
        ...
        // });
        $(".inline-readmore").click(function(e) {
            e.preventDefault();
            var fullStringSpan = $(this).siblings(".full-string-span");
            fullStringSpan.toggle();
            var moreText = $(this).find(".more-text");
            var lessText = $(this).find(".less-text");
            moreText.toggle();
            lessText.toggle();
        });
    });
...
<script>
    ...
    // <!--Start::City Add Function-->
        $(document).on('click', 'button[data-role=addFaq]', function() {
            validator.resetForm();
            $('#charCountQue').text(0 + ' / ' + 1000);
            $('#charCount').text(0 + ' / ' + 1000);
            $('#charCountQue').removeClass('badge-danger').addClass('badge-primary');
            $('#charCount').removeClass('badge-danger').addClass('badge-primary');
            $('#kt_modal_add_faq_form')[0].reset();
            $('#model_heading').html('Add FAQ');
            $('#kt_modal_add_faq').modal('toggle');
        });
    // <!--End::City Add Function-->

    // <!--Start::City Edit Function-->
        $(document).on('click', 'a[data-role=editFaq]', function() {
            validator.resetForm();
            var id = $(this).data('id');
            ...
            $('#model_heading').html('Edit Question Answer');
            $('#kt_modal_add_faq').modal('toggle');
        });
    ...
    // <!--Start::Form Validation-->
        ...
        var validator = $('form[id="kt_modal_add_faq_form"]').validate({
            rules: {
                title: {
                    noSpace: true
                },
            ...
            submitHandler: function(form) {
                var formData = new FormData($('#kt_modal_add_faq_form')[0]);
                $.ajax({
                    ...
                    success: function(data) {
                        if(data == 1) {
                            $('#kt_modal_add_faq').modal('toggle');
                            swal.fire({
                                text: "Question answer has been successfully submitted!",
                                icon: "success",
                                buttonsStyling: !1,
    ...
    // <!--Start::City Delete Function-->
        $(document).on('click', 'a[data-role=deleteEmailContaint]', function() {
            var id = $(this).data('id');
            Swal.fire({
                text: "Are you sure you want to delete Question and answer?",
                icon: "warning",
                showCancelButton: !0,
                buttonsStyling: !1,
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if(data == 1) {
                            Swal.fire({
                                text: "You have deleted Question and Answer!.",
                                icon: "success",
                                buttonsStyling: !1,
                                confirmButtonText: "Ok, Got it!",
                                customClass: {
